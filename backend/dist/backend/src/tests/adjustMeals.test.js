...
describe("adjustment logic", () => {
    ...
    const countMeals = (prefs) => prefs.reduce((sum, r) => sum + r.servings, 0);
    it("increase servings to match total meals", () => {
        ...
        while (total < totalMeals) {
            const best = servings.reduce((a, b) => {
                const scoreA = (0, nutritionCalcs_1.getNutritionDiff)(goals, { calories: 0, protein: 0, carbs: 0 }, nutritionMap.get(a.spoonacularId));
                const scoreB = (0, nutritionCalcs_1.getNutritionDiff)(goals, { calories: 0, protein: 0, carbs: 0 }, nutritionMap.get(b.spoonacularId));
    ...
    it("decrease servings to match total meals", () => {
        ...
        while (total > totalMeals) {
            const worse = servings.reduce((a, b) => {
                const excessA = (0, nutritionCalcs_1.getExcess)(goals, { calories: 0, protein: 0, carbs: 0 }, nutritionMap.get(a.spoonacularId));
                const excessB = (0, nutritionCalcs_1.getExcess)(goals, { calories: 0, protein: 0, carbs: 0 }, nutritionMap.get(b.spoonacularId));
