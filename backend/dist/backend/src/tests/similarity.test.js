...
describe("recipe similarity", () => {
    it("calculate weighted jaccard similarity score", () => {
        ...
        const result = (0, similarity_1.recipeSimilar)(a, b);
...
describe("ingredient overlap", () => {
    it("calculate jaccard similarity score for ingredients", () => {
        ...
        const result = (0, similarity_1.ingredientOverlap)(setA, setB);
...
describe("cosine similarity", () => {
    it("calculates cosine similarity for users", () => {
        ...
        const result = (0, similarity_1.cosineSimilarity)(a, b);
        expect(result).toBeCloseTo(4 / (Math.sqrt(5) * 3));
    });
    it("results to 0 if a user has no interactions", () => {
        const a = new Map([["1", 1]]);
        const b = new Map();
        expect((0, similarity_1.cosineSimilarity)(a, b)).toBe(0);
    });
});
describe("top 5 users", () => {
    const recipe = (id, ingredients) => ({ id, ingredients });
    const userA = {
        id: "1",
        username: "userA",
    ...
    it("return top 5 users sorted by similarity", () => {
        var _a;
        const result = (0, similarity_1.top5Users)(userA, [userB, userC, userD, userE, userF]);
        expect(result.length).toBe(3);
        expect(result[0].user.username).toBe("userB");
        expect(result[0].finalScore).toBeGreaterThan((_a = result[1]) === null || _a === void 0 ? void 0 : _a.finalScore);
    });
    it("does not include users with 0 similarity", () => {
        const result = (0, similarity_1.top5Users)(userA, [userF]);
